...
func main() {
	...
	// 生成 replace 请求
	req, err := client.NewRequest(tools.ZoneId, "tb_online_list", cmd.TcaplusApiListReplaceReq)
	if err != nil {
	...
	// （非必须，默认为 0）replace 请求设置 2 时将返回此次更新的记录，3 返回更新前的记录， 0 1 不返回记录
	...
	logger.INFO("listreplace success")
