...
func main() {
	...
	// 生成 delete 请求
	req, err := client.NewRequest(tools.ZoneId, "tb_online_list", cmd.TcaplusApiListDeleteBatchReq)
	if err != nil {
	...
	// （非必须，默认为 0） 请求设置 3 时将返回此次删除的记录，0 1 2不返回记录
	...
	logger.INFO("listdeletebatch success")
